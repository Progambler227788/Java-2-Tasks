...
class Spreadsheet {
    ...
    private double evaluateFormula(Formula formula) throws Exception {
        ...
        List<Token> postfixTokens = ShuntingYard.infixToPostfix(tokens);  // Use the correct method name
    ...
    private void processFormulas() throws Exception {
        for (Cell cell : cells.values()) {
            if (cell.getContent() instanceof Formula) {
                evaluateFormula((Formula) cell.getContent());
            }
        }
    }
    ...
    public void printSpreadsheet() {
        ...
        for (Coordinate key : cells.keySet()) {
            ...
            if (column.charAt(0) > maxColumn) maxColumn = column.charAt(0);
        }
        ...
        for (int row = 1; row <= maxRow; row++) {
            ...
            for (char col = 'A'; col <= maxColumn; col++) {
                String key = col + Integer.toString(row);
                try {
                    String content = getCellContent(key);
                    if (content.startsWith("=")) {
                        double value = getCellValue(key);
